...
} from "./transformToCodeWithMustache";
...
describe("transformToCodeWithMustache", (): void => {
  ...
  beforeAll(() => {
    ...
    templates = {
      ...
      type: readFileSync(
        path.join(DEFAULT_TEMPLATE_PATH, "type.mustache"),
        "utf-8"
      )
    };
  });
  ...
  it("calls Mustache.render with the default templates for the typescript target", () => {
    ...
    expect(Mustache.render).toBeCalledWith(
      templates.class,
      viewData,
      templates
    );
  ...
  it("adds passed mustache options to the viewData when calling render", () => {
    ...
    expect((Mustache.render as jest.Mock).mock.calls[0][1].name).toBe(
      "MyCustomName"
    );
  });
